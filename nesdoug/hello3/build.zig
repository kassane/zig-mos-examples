...
pub fn build(b: *std.Build) void {
    ...
    const nesdoug_mod = blk: {
        const tc = b.addTranslateC(.{
            .root_source_file = .{ .cwd_relative = b.fmt("{s}/mos-platform/nes/include/nesdoug.h", .{sdk}) },
            .target = target,
            .optimize = .ReleaseFast,
            .link_libc = false,
        });
    ...
    exe.root_module.linkSystemLibrary("nesdoug", .{ .use_pkg_config = .no });
    exe.root_module.linkSystemLibrary("crt0", .{ .use_pkg_config = .no });
    exe.setLinkerScript(wrapper_ld);

    const install = b.addInstallArtifact(exe, .{ .dest_sub_path = "hello.nes" });
